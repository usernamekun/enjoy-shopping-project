...
export default {
  ...
  computed: {
    ...
    // 使用props传递数据 如果是通过服务器获取的数据，而且是个多级数据，需要读取某个数据里面的某个属性时 必须要有下面的操作
    skuImageList(){
  ...
  methods: {
    ...
    async addShopCart(){
      ...
      // 2,服务器写入成功
      // try...catch 这条语句是 如果成功了就执行try里面的语句  如果失败了就执行catch中的语句
      try {
          // 下面这行代码说白了就是调用了仓库中addOrUpdateShopCart这个方法，这个方法加上async，返回的一定是promise
        await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum})
        // 路由跳转
        // 在路由跳转的时候还需要将商品的信息带给下一个路由
        // 一些简单的skuNum数据，通过query形式给路由组件传递过去
